                ...
                <div class="row">
                    <div class="col-md-8 blog-pull-right">
                        <div class="single-service">
                            <img src="{{ asset('web_images/' . $blog?->image) }}" alt="">
                            <h3 class="text-theme-colored line-bottom text-theme-colored">{{ $blog?->title }}</h3>
                            <p>{!! $blog?->details !!}</p>
                            ...
                            <div class="widget">
                                ...
                                <div class="services-list">
                                    <ul class="list list-border">
                                        @foreach ($blog_category as $val)
                                            <li class="{{ $blog->category_id == $val->id ? 'active' : '' }}"><a
                                                    href="{{ route('web.our_blog', ['slug_name' => $val->slug_name]) }}">{{ $val->name }}</a>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
